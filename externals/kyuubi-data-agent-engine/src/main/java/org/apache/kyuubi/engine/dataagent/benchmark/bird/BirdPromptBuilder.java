...
public final class BirdPromptBuilder {
  ...
  public static String build(BenchmarkExample ex, String dialectName) {
    ...
    sb.append("Workflow\n")
        ...
        .append(
            "- When confident, call submit_sql exactly once with your final answer. ONLY the SQL submitted that way is scored.\n\n");

    sb.append("Rules for the final query\n")
        .append(
            "- SELECT only the columns the question explicitly asks for, in the order the question mentions them.")
        .append(" Do NOT include columns used only in WHERE, ORDER BY, GROUP BY, or HAVING.\n")
        .append(
            "- If the question asks \"how many / how much / total / average / sum / maximum / minimum / count / percentage / ratio\",")
        .append(
            " your SELECT MUST use an aggregate (COUNT/SUM/AVG/MAX/MIN) — do not return raw rows.\n")
        .append(
            "- Add LIMIT 1 when the question asks for a single item (\"the most\", \"the highest\", \"the top\", \"the oldest\", \"the best\").\n")
        .append(
            "- Use SELECT DISTINCT when the question asks to \"list\" entities and your JOINs can produce duplicates.\n")
        .append(
            "- Prefer exact string matches over LIKE unless the question uses \"contains\" / \"starts with\" / \"ends with\".\n\n");

    if (ex.evidence() != null && !ex.evidence().isEmpty()) {
      sb.append("Domain rule (apply this when writing the final query):\n")
          .append(ex.evidence().trim())
          .append("\n\n");
    }
