 ...
 *
 * <p>This tool delegates the actual JDBC execution to {@link RunSelectQueryTool} so the agent
 * still sees the query's result set (useful for catching typos), then stashes the submitted SQL.
 * The benchmark runner prefers the stashed SQL over the last-successful heuristic and only falls
 * back to the heuristic if the agent never called this tool.
 *
 * <p>Main-scope package instead of plain benchmark-scope because the tool reuses
 * {@link RunSelectQueryTool} and shares its {@link DataSource} per question.
 */
public class SubmitSqlTool implements AgentTool<SqlQueryArgs> {
    ...
    public String execute(SqlQueryArgs args) {
        ...
        String result = runner.execute(args);
