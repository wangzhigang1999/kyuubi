...
public class ToolSchemaGeneratorTest {
  ...
  public void testSqlQueryArgsSchema() {
    ...
    assertNull("$schema key should be stripped", schema.get("$schema"));
  ...
  public void testNoRequiredFields() {
    ...
    // required key should be absent or empty
    @SuppressWarnings("unchecked")
    List<String> required = (List<String>) schema.get("required");
  ...
  @Test
  public void testSchemaKeyIsStripped() {
    Map<String, Object> schema = ToolSchemaGenerator.generateSchema(SqlQueryArgs.class);
    assertFalse("$schema should not be present", schema.containsKey("$schema"));
  }
